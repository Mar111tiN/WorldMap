...
function setUp(data) {
  ...
  setColorScales();
  makeMap(geoData);
  setMapColor();    //  STATE.year --> |svg color|-  apply fill to .country paths
  setTT();          // tooltip
...
//==================YEAR RANGES FOR SCALES AND YEAR SELECTOR=================
  ...
  function updateYearSelect() {
    ...
    let yearKeyPresent = isYearKey(state.key) || isYearKey(scatterState.x) || isYearKey(scatterState.y) || isYearKey(scatterState.r);
    ...
    // hide 
    d3.select('.year').classed('hidden', !yearKeyPresent);
  }
...
//===========================MAP ADJUSTMENT --> MOVE TO globe.js!!
  ...
  // get fill color from elem data and state.key --> can be used for scatter as well
  function setElemColor(d) {
    let key = state.key;
    var cScale = colorScales[key];       
  // if (key == 'emissionsPerCapita') debugger;  
    let color = (d.properties[key]) 
    ? (isYearKey(key)) 
